...
const monsterGrowlAudio = new Audio('assets/music/eviljest.mp3');
const monsterBiteAudio = new Audio('assets/music/eviljest.mp3');
...
const player = new Player( canvas.width/2 - this.width/2, platformYPosition - this.height + 17);
...
function createPowerBall() {
    ...
    return new PowerBall(powerBallPositionX, player.yPosition, player.direction, player.strength);
}
...
window.onload = () => {
    ...
    function updateGame() {
        ...
        if(snack != undefined) {
            ...
            // collision of player with snack, update health of player.
            if (
                player.xPosition < snack.xPosition + snack.width - 50 && 
                player.xPosition + player.width - 50 > snack.xPosition &&
                player.yPosition < snack.yPosition + snack.height
                ) {
                    if(player.health < 100) {
                        player.health += snack.refill;
                        if(player.health > 100) {
                            player.health = 100;
                        }
                    }
        ...
        for(let i = 0; i < knifeArr.length; i++) {
            ...
            if(knifeArr[i].xPosition + knifeArr[i].width < 0 || knifeArr[i].xPosition > canvas.width) {
                knifeArr.splice(i,1);
                continue;
            }

            // collision of knife with monster --> to kill monster with knife
            for(let j = 0; j < monstersArray.length; j++) {
                if (
                    knifeArr[i].xPosition < monstersArray[j].xPosition + monstersArray[j].width - 32 &&
                    knifeArr[i].xPosition + knifeArr[i].width > monstersArray[j].xPosition
                ) {
                    monstersArray[j].receiveAttack(knifeArr[i].strength);
                    knifeArr.splice(i, 1);
        ...
        for(let i = 0; i < monstersArray.length; i++) {
            ...
            monstersArray[i].changeDirection(player.xPosition, player.width);

            // player collision with monsters
            if (
                player.xPosition < monstersArray[i].xPosition + monstersArray[i].width - 32 &&
                player.xPosition + player.width - 32 > monstersArray[i].xPosition
            ) {
                ...
                // when monster attack to player
                if (monstersArray[i].readyToAttack) {
                    player.receiveAttack(monstersArray[i].strength);
                    ...
                    // game over when player's health zero on monster attack.
                    if (player.health <= 0) {
                        isGameOver = true;
                    }
                }
